...
export default function Expenses({ ctx }) {
  ...
  let balanceText = t('All square');
  let settle = null;
  if (members.length === 2) {
    const [a, b] = members;
    if (bal[a] > 0.5) { balanceText = t('{a} owes {b} {amount}', { a: nameOf(trip, b), b: nameOf(trip, a), amount: money(bal[a], home) }); settle = { from: b, to: a, amount: bal[a] }; }
    else if (bal[b] > 0.5) { balanceText = t('{a} owes {b} {amount}', { a: nameOf(trip, a), b: nameOf(trip, b), amount: money(bal[b], home) }); settle = { from: a, to: b, amount: bal[b] }; }
  }
  const doSettle = () => {
    if (!settle) return;
    addRow(trip.id, 'expenses', {
      title: t('Settled up: {a} paid {b}', { a: nameOf(trip, settle.from), b: nameOf(trip, settle.to) }),
  ...
  return (
    <>
      <main className="screen">
        ...
        <div className="card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14, borderRadius: 20 }}>
          ...
          {cats.length > 0 && (
            <>
              ...
              <div className="legend" role="list" aria-label={t('Spending by category')}>
                {cats.map((c) => (
        ...
        {members.length === 2 && (
          <div className="card row" style={{ padding: 14, gap: 12 }}>
            <div className="stack grow" style={{ gap: 2 }}>
              ...
              <span className="small muted">{members.map((m) => t('{name} paid {amount}', { name: nameOf(trip, m), amount: money(paid[m], home) })).join(' · ')}</span>
            </div>
            {settle && <button className="btn sm outline" style={{ height: 44 }} onClick={doSettle}>{t('Settle up')}</button>}
          </div>
        )}
        ...
        {!all.length && <div className="empty">{t('Add a price when you put something on the itinerary and it shows up here. Anything else, add with the + button.')}</div>}
        ...
        {keys.map((k) => {
          ...
          return (
            <section key={k} className="section" style={{ gap: 8 }}>
              <div className="between">
                <h2 className="eyebrow">{k === 'before' ? t('Before the trip') : k === 'after' ? t('After the trip') : dayLabel(days, k)}</h2>
      ...
      <Fab label={t('Add an expense')} onClick={() => open({ kind: 'expForm' })} />
    </>
  );
}
