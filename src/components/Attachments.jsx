...
export default function Attachments({ ctx, parentId, label = 'Add ticket or photo', pending, onPending }) {
  ...
  const view = async (a) => {
    ...
    if (!url) { setError('That file could not be loaded. Check your connection.'); if (w) w.close(); return; }
    ...
    <div className="stack" style={{ gap: 8 }}>
      {list.map((a) => (
        <div key={a.id} className="row card" style={{ padding: '4px 4px 4px 12px', gap: 8, borderRadius: 12 }}>
          ...
          <button className="iconbtn clear" aria-label={'Remove ' + a.name} onClick={() => removeAttachment(trip.id, a.id)}><Icon d="trash" size={16} /></button>
        </div>
      ))}
      {pending && (
        <div className="row card" style={{ padding: '12px', gap: 8, borderRadius: 12 }}>
          <Icon d="file" size={18} color="var(--muted)" /><span className="grow small" style={{ fontWeight: 600 }}>{pending.meta.name} (saved with the expense)</span>
        </div>
      )}
      ...
      <button type="button" className="btn sm dashed" onClick={() => input.current.click()} disabled={busy} style={{ alignSelf: 'flex-start' }}>
        <Icon d="clip" size={15} stroke={2} />{busy ? 'Uploading…' : label}
      </button>
      {error && <p className="error" role="alert">{error}</p>}
      {preview && (
        <>
          <button className="backdrop" style={{ position: 'fixed', zIndex: 40, background: 'rgba(0,0,0,0.85)' }} aria-label="Close preview" onClick={() => setPreview(null)} />
          <img src={preview} alt="Attachment" style={{ position: 'fixed', zIndex: 41, left: 12, right: 12, top: '50%', transform: 'translateY(-50%)', maxWidth: 'calc(100% - 24px)', maxHeight: '80%', objectFit: 'contain', margin: '0 auto', borderRadius: 12 }}
